...
// 2 Loops
int findDuplicate_2loops(int nums[], int len)
{
    for (int i = 0; i < len; i++)
    {
...
int findDuplicate(int nums[], int len)
{
    int cnt[len + 1];
    for (int i = 0; i < len; i++)
    {
...
// Visited
int findDuplicate_mark(vector<int> nums, int len)
{
    for (int num : nums)
    {
        int idx = abs(num);
...
int findDuplicate_sort(vector<int> nums, int len)
{
...
// Index Sort
// n + 1 numbers in n.
int findDuplicate_index_sort(int nums[], int len)
{
    for (int i = 0; i < len;)
    {
        int n = nums[i];
...
int findDuplicate_bs(int nums[], int len)
{
    int low = 1;
    int high = len - 1;
    while (low < high)
...
// For example, the ith digit, note that in the nums, the sum of all the elements whose ith digit is 1 is x as convert the number to binary.
...
// Bit(1)	1	0	1	1	1	3	2
// Bit(2)	0	0	1	0	0	1	1
// From the table, we found that only the 1th bit x > y, so after bitwise restoration target=(010)2 = (2)^10, which is the answer.
...
// If the i-th bit of the number to be replaced is0, and the i-th bit of target is00: x remains unchanged, satisfying x ≤ y.
...
// Take the array[1, 3, 4, 2] as an example,
// the index of this array is[0, 1, 2, 3], we can map the index to the nums[n]

// 0 → 1 → 3 → 2 → 4 → 3→ 2 Start from nums[n] as a new index, and so on, until the index exceeds the bounds. This produces a sequence similar to a linked list.
// 0 → 1 → 3 → 2 → 4 → null If there are a repeated numbers in the array, take the array [1,3,4,2,2][1,3,4,2,2][1,3,4,2,2] as an example,
// 0 → 1 → 3 → 2 → 4 → 3→ 2 → 4 → 2 Similarly, a linked list is like that:
// 0 → 1 → 3 → 2 → 4 → 2→ 4 → 2   Here 2→4 is a cycle, then this linked list can be abstracted as the following figure:
